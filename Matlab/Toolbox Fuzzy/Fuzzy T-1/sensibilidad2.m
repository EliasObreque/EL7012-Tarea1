function [p, indice]=sensibilidad(yent,Xent,Ytest,Xtest,reglas)
%keyboard
...
%keyboard
muu=zeros(length(Xtest(1,:)),length(a(:,1)),length(Xtest(:,1)));
for i=1:length(Xtest(1,:))
    for r=1:length(a(:,1))
        for j=1:length(Xtest(:,1))
            muu(i,r,j)=exp(-0.5*(a(r,i)*(Xtest(j,i)-b(r,i)))^2);
        end
    end
end
...
w=ones(length(Xtest(:,1)),length(a(:,1)));
for j=1:length(Xtest(:,1))
    for r=1:length(a(:,1))
        for i=1:length(Xtest(1,:))
            aux=muu(i,r,j);
...
c=zeros(length(Xtest(1,:)),length(a(:,1)),length(Xtest(:,1)));
for i=1:length(Xtest(1,:))
    for r=1:length(a(:,1))
        for j=1:length(Xtest(:,1))
            c(i,r,j)=-(a(r,i)*(Xtest(j,i)-b(r,i)))*a(r,i);
        end
    end
end
%se obtienen las salidas para cada regla
%yr jxr
yr=[ones(length(Xtest(:,1)),1),Xtest]*g';

%se obtienen los chi para cada candidata
%chi jxi
chi=zeros(length(Xtest(:,1)),length(Xtest(1,:)));
suma1=0;
suma2=0;
suma3=0;
suma4=0;
