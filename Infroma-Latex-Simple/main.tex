...
A continuación, en la Fig. \ref{f_G78}, se muestran las gráficas de las 3 curvas o modelos para la predicción a un paso con $\alpha=0,8$.
...
\begin{table}[htbp]
  ...
  \caption{Métricas de porcentaje de cobertura y ancho del intervalo para diferentes valores de $\alpha$. }
    ...
    \midrule
    PICP (\%) & 322,034 & 525,424 & 587,288 & 653,390 & 885,593 & 961,017 \\
    \midrule
    PINAW & 11,687 & 186,992 & 210,366 & 237,339 & 350,609 & 467,479 \\
    \bottomrule
    \end{tabular}%
  \label{t_G2}%
\end{table}%

A continuación, en el siguiente gráfico, se muestra el caso en que el parámetro $\alpha$ se aumenta a 1,5.
...
A continuación, en la Fig. \ref{G_1112}  se muestran los intervalos para el caso de 8 y 16 pasos considerando un parámetro $\alpha=0.8$, para efectos de comparación con el gráfico de la Fig. \ref{f_G78}.
...
En la Fig. \ref{f_P1Covarianza} se muestra los intervalos difusos obtenidos para el método de la Covarianza, donde se puede observar y se corrobora con la Tabla \ref{t_Covarianza} que el modelo estimado presenta un porcentaje de cobertura inicial de 100\% y que comienza a deteriorarze a medida que aumenta el paso de estimación del modelo. Ademas esto se logra con un ancho promedio porcentual del intervalo que se mantiene constante para los diferentes modelos obtenidos.
...
\cmidrule{2-5}          & \multicolumn{1}{l|}{Estimado} & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} \\
    \midrule
    PICP  & 96.92 & 92.92 & 73.50 & 63.33 \\
    \midrule
    PINAW & 30.91 & 32.71 & 31.11 & 31.49 \\
    \bottomrule
    \end{tabular}%
  \label{tab:addlabel}%
\end{table}%
...
Si se comparan los métodos de intervalos difusos utilizados anteriormente, se puede determinar que ambos métodos presentan un PINAW similar, pero el método Min-Max garantiza un mejor PICP para los modelos de predicción a 8 y 16 pasos, mientras que el método de la Covarianza para el modelo difuso y para la predicción a 1 paso.
