...
En la Fig. \ref{f_G123}, se muestran las gráficas de la predicción a un paso en cada conjunto de datos, a saber, conjunto de entrenamiento, prueba y validación. Esto es solo a modo de comparación puesto que finalmente, lo relevante de nuestro modelo, es el comportamiento o bondad de ajuste en el conjunto de validación. Evidentemente que los datos de los parámetros usados en las predicciones, para todos los conjuntos, son aquellos obtenidos con el conjunto de entrenamiento, sin embargo, la matriz de represores y salidas cambian dependiendo del conjunto que se esté usando para predicción.
...
\begin{table}[htbp]
  ...
  \caption{Errores o Métricas de bondad de ajuste a 1 paso}
    ...
    \midrule
    RMSE  & 0.0869 & 0.0559 & 0.0954 \\
    \midrule
    MAPE  & 162.2793 & 220.0863 & 238.2150 \\
    \midrule
    MAE   & 0.3569 & 0.3465 & 0.6020 \\
    \bottomrule
    \end{tabular}%
  \label{t_NoLinealp1}%
\end{table}%
...
\textbf{Construcción de Intervalos para el modelo lineal}
...
Así entonces, podemos determinar los modelos por arriba y por abajo del modelo clásico de valor esperado, los cuales nos definen el intervalo.

En la Fig. \ref{f_G78}, se muestran las gráficas de las 3 curvas o modelos para la predicción a un paso con $\alpha=0,8$.
...
Ahora bien, podemos ir ajustando el valor de $\alpha$ para aumentar o disminuir el porcentaje de cobertura de las muestras de validación.

En la Tabla \ref{t_G2}, se muestran las métricas PICP y PINAW las cuales miden el porcentaje de cobertura y ancho del intervalo respectivamente, para diferentes valores de $\alpha$ de la predicción a 1 paso.

% Table generated by Excel2LaTeX from sheet 'Sheet1'
\begin{table}[htbp]
  \centering
  \caption{Métricas de porcentaje de cobertura y ancho del intervalo para diferentes valores de $\alpha$. }
    \begin{tabular}{|p{4.055em}|r|r|r|r|r|r|}
    \toprule
    Parámetro $\alpha$ & \multicolumn{1}{p{4.055em}|}{0,5} & \multicolumn{1}{p{4.055em}|}{0,8} & \multicolumn{1}{p{4.055em}|}{0,9} & \multicolumn{1}{c|}{1} & \multicolumn{1}{p{4.055em}|}{1,5} & \multicolumn{1}{c|}{2} \\
    \midrule
...
En la Fig. \ref{f_G910} se muestra el caso en que el parámetro $\alpha$ se aumenta a 1.5.
...
\begin{table}[htbp]
  ...
  \caption{Métricas de porcentaje de cobertura y ancho del intervalo para diferentes valores de $\alpha=0,8$ considerando intervalos de predicción a 1, 8 y 16 pasos respectivamente.}
    \begin{tabular}{|p{8.61em}|r|r|r|}
    \toprule
    \textbf{Parámetro $\alpha=0,8$} & \multicolumn{3}{p{12.165em}|}{\textbf{0,8}} \\
    \midrule
    \multicolumn{1}{|r|}{} & \multicolumn{1}{p{4.055em}|}{1 paso} & \multicolumn{1}{p{4.055em}|}{8 pasos} & \multicolumn{1}{p{4.055em}|}{16 pasos} \\
    \midrule
    PICP (\%) & 52.5424 & 52.7966 & 52.7966 \\
    \midrule
    PINAW & 18.6992 & 18.4244 & 18.7348 \\
    \bottomrule
    \end{tabular}%
  \label{t_G3}%
\end{table}%
...
En la Tabla \ref{t_MDjp} se establece una comparación entre los modelos obtenidos, como se puede comprobar a medida que aumenta el paso de la estimación el desempeño del modelo difuso se deteriora.
...
\begin{table}[htbp]
  ...
  \caption{Desempeño del Modelo difuso a j-pasos}
    \begin{tabular}{|l|r|r|r|}
    \toprule
    Índices & \multicolumn{3}{c|}{Modelos} \\
\cmidrule{2-4}          & \multicolumn{1}{p{6em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{6.39em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{5.445em}|}{Predicción a 16 pasos} \\
    \midrule
    RMSE  & 0.1938 &  0.7077 & 0.8847 \\
    \midrule
    MAPE  & 67.71 & 159.78 & 169.81 \\
    \midrule
    MAE   & 0.1344 &  0.5076 & 0.6814 \\
    \bottomrule
    \end{tabular}%
  \label{t_MDjp}%
\end{table}%

\subsubsection{Intervalos Difusos}
...
\begin{equation}
PINAW=\frac{1}{NR}\sum_{k=1}^{N}(\hat{y}_u(k)-\hat{y}_l(k)) \qquad donde\quad R=max(y(k))-min(y(k))
\label{e_PINAW}
\end{equation}
...
En la Fig. \ref{f_P1Covarianza} se muestra los intervalos difusos obtenidos para el método de la Covarianza, donde se puede observar y se corrobora con la Tabla \ref{t_Covarianza} para un $\alpha=2$ que el modelo estimado presenta un porcentaje de cobertura inicial de 100\% y que comienza a deteriorarze a medida que aumenta el paso de estimación del modelo. Ademas esto se logra con un ancho promedio porcentual del intervalo que se mantiene constante para los diferentes modelos obtenidos.
...
\begin{table}[htbp]
  ...
  \caption{Intervalos de predicción. Método MinMax}
    ...
    \toprule
    Índices & \multicolumn{3}{c|}{Modelos} \\
\cmidrule{2-4}          & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} \\
    \midrule
    PICP  & 96.92  & 73.50 & 63.33 \\
    \midrule
    PINAW & 30.91  & 31.11 & 31.49 \\
    \bottomrule
    \end{tabular}%
  \label{tab:addlabel}%
\end{table}%
...
Si se comparan los métodos de intervalos difusos utilizados anteriormente, se puede determinar que ambos métodos presentan un PINAW similar, pero el método Covarianza garantiza un mejor PICP para los modelos de predicción a 1, 8 y 16 pasos.
...
\begin{itemize}
	...
	\begin{figure}
		\centering
		\captionsetup{justification=centering}
		 \includegraphics[width=0.5\textwidth]{imag/redes/pred_red.eps}
...
\end{figure}
	...
	\end{table}
\end{itemize}
\subsubsection{Comentarios sobre los modelos}
\newpage
\subsection{Intervalos de predicción por covarianza}
\subsubsection{Modelo lineal}
\subsubsection{Modelo difuso Takagi-Sugeno}
\subsubsection{Modelo de red neuronal}
Del modelo matemático de la ecuación \ref{mat_red}, es posible reescribir la ecuación separando las capas, es decir,
\begin{align}
...
\cmidrule{2-10}          & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} \\
    \midrule
    RMSE  & 0.0954 & 0.0789 & 0.0964 & 0.1938 & 0.7077 & 0.8847 &       &       &  \\
    \midrule
    MAPE  & \multicolumn{1}{r|}{238.22} & \multicolumn{1}{r|}{226.60} & \multicolumn{1}{r|}{235.33} & 67.71 & 159.78 & 169.81 &       &       &  \\
    \midrule
    MAE   & 0.6020 & 0.5805 & 0.5944 & 0.1344 & 0.5076 & 0.6814 &       &       &  \\
    \bottomrule
    \end{tabular}%
  \label{t_Comp}%
\end{table}%
...
\cmidrule{2-10}          & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 1 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 8 pasos} & \multicolumn{1}{p{4.055em}|}{Predicción a 16 pasos} \\
    \midrule
    PICP  & 52.542 & 52.797 & 52.797 & 100   & 84.67 & 72    &       &       &  \\
    \midrule
    PINAW & 18.6992   & 18.4244   & 18.7348   & 27.59  & 29.55 & 28.1  &       &       &  \\
    \bottomrule
    \end{tabular}%
  \label{t_CompCov}%
\end{table}%
...
Tabla xx: Errores ó Métricas de bondad de ajuste para predicción resumida en los conjuntos de validación para los tres modelos obtenidos.
